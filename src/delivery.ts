...
async function drainSession(session: Session): Promise<void> {
  ...
  for (const msg of pending) {
    try {
      ...
      // Pause the typing indicator after a real user-facing message lands on
      // the user's screen, so the client has time to visually clear the
      // indicator before the next heartbeat tick brings it back. Skip the
      // pause for internal traffic (system actions, agent-to-agent routing)
      // — the user doesn't see those and shouldn't get a gap in their typing
      // indicator for them. turn_metadata is pure observability bookkeeping
      // (never delivered to a channel — see deliverMessage's branch) and
      // gets the same exclusion.
      if (msg.kind !== 'system' && msg.kind !== 'turn_metadata' && msg.channelType !== 'agent') {
        ...
        if (msg.kind !== 'task_log') {
          ...
          // Post-delivery hooks: user-facing rows only, same exclusions as
          // the fan above. Hooks are decoration: failures log and can never
          // affect delivery or retries.
...
async function deliverMessage(
  ...
  if (msg.kind === 'turn_metadata') {
    ...
    try {
      const inboundRow = await withExistingMailboxSession(session.agent_group_id, session.id, (mailbox) =>
        mailbox.getInboundMessageById(msg.inReplyTo!),
      );
      if (!inboundRow) {
      ...
      await logInteraction({
        ...
        channel: inboundRow.channelType ?? undefined,
