...
export interface DockerDriverOptions extends MountPolicy {
  ...
  /**
   * Extra `-e` argv appended after both composed and contributed env, so it
   * can never be silently clobbered by a gateway's own contributed vars.
   * Driver-private the same way network topology is (see `networkArgsFor`):
   * this seam exists for realizations that need an escape hatch tied to the
   * same private decision (e.g. a proxy-bypass exemption that only applies
   * outside egress lockdown), not for composition to reach into.
   */
  envArgsFor?: (spec: SessionSpec) => string[];
}
...
export class DockerSessionDriver implements SessionDriver {
  ...
  async prepare(spec: SessionSpec): Promise<SessionHandle> {
    ...
    args.push(...(this.opts.envArgsFor?.(spec) ?? []));
