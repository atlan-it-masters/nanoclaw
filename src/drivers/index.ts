...
import { DATA_DIR, GROUPS_DIR, NANOCLAW_NO_PROXY_HOSTS } from '../config.js';
 ...
 * alias there, so an http(s)-type MCP server URL pointing at
 * host.docker.internal (e.g. a companion service like sentinelone-mcp) would
 * otherwise get silently routed through OneCLI's HTTPS_PROXY/HTTP_PROXY
 * (contributed by the gateway provider) and fail to resolve there. Exempt it
 * so those requests go direct. Same failure mode hits any other host only the
 * AGENT container (not the OneCLI gateway container) can route to — e.g. an
 * on-prem MCP target on a private LAN IP (a stdio server's env, not just
 * http-type URLs, can name one — Wazuh's WAZUH_API_HOST is a bare IP) —
 * NANOCLAW_NO_PROXY_HOSTS covers those.
 *
 * Deliberately absent under lockdown: exempting a host there would let agent
 * traffic bypass the mandatory gateway, defeating the lockdown's whole point.
 * Calling `ensureEgressNetwork()` again here (already called once inside
 * `dockerNetworkArgs`) is redundant but cheap and idempotent — it never
 * mutates on a second call once the network/attachment already exist.
 */
function dockerNoProxyEnvArgs(): string[] {
  if (ensureEgressNetwork()) return [];
  const noProxyHosts = ['host.docker.internal', ...NANOCLAW_NO_PROXY_HOSTS].join(',');
  return ['-e', `NO_PROXY=${noProxyHosts}`, '-e', `no_proxy=${noProxyHosts}`];
}

registerSessionDriver(
  DEFAULT_DRIVER_KIND,
  (policy) =>
    new DockerSessionDriver({ ...policy, networkArgsFor: dockerNetworkArgs, envArgsFor: dockerNoProxyEnvArgs }),
);
