...
/**
 * Look up an inbound row's content/timestamp/channel by its message id.
 * Used by the turn_metadata observability branch (delivery.ts) to recover
 * the question text a logged answer is attributed to. Returns undefined if
 * the row doesn't exist (e.g. pruned) rather than throwing — the caller
 * drops the observability row in that case instead of failing delivery.
 */
export function getInboundMessageById(
  db: Database.Database,
  messageId: string,
): { content: string; timestamp: string; channelType: string | null } | undefined {
  const row = db.prepare('SELECT content, timestamp, channel_type FROM messages_in WHERE id = ?').get(messageId) as
    | { content: string; timestamp: string; channel_type: string | null }
    | undefined;
  return row && { content: row.content, timestamp: row.timestamp, channelType: row.channel_type };
}
